...
- [ ] **OpenSearch 直連掃描若讀到的筆數少於索引宣稱的，該索引必須失敗** —— 不可
      ...
      測試都不可能抓到它。*
- [ ] **取消匯出必須標成「已取消」，絕不可標成完成** —— 在 UI 中途按取消：徽章要
      顯示低調的「已由使用者取消」（兩種語言，不可直接顯示英文原字串），進度條停在
      中斷處而不是跳到 100%，備註要說明已寫出的封存會保留。*被取消的執行過去會顯示
      成「完成、100%、0 筆」，旁邊卻寫著已寫出 54.2 MB——真實客戶無從判斷資料有沒有
      遺失。*
- [ ] **筆數必須在取消後保留** —— 取消後顯示的筆數要與執行中計數器最後的數字相符，
      不是 0。可到歸檔清單用該時間範圍交叉核對。*`messages_total` 只在單元完整結束後
      才累加，而被中斷單元的封存確實在磁碟上、也在資料庫裡，必須計入。*
- [ ] **被取消的執行不送完成通知** —— 也不送錯誤通知。取消是刻意的行為。
