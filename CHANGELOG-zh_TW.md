...
- **匯入前一鍵清除目標索引（選用）。** 若匯入的索引組中仍留有舊資料，舊的欄位型別對應
  也仍然生效 —— 這正是為什麼早期歸檔留下的錯誤型別對應，會一直讓重新匯入失敗，即使
  歸檔本身已經沒問題。匯入對話框新增一個預設摺疊的**「匯入前清除目標索引」**區塊，
  會列出目標的索引組，並顯示**目前有幾個索引、總共佔用多少容量**，預設選取目標的
  預設索引組。清除時會**先輪替寫入索引**，再刪除輪替後留下的舊索引，因此目標會保留
  一個全新的空白寫入索引，仍可正常接收資料。

  因為這會刪除資料，設有以下保護：
  - Graylog 內建的事件索引組（`gl-events`、`gl-system-events`）永遠不會列出，即使
...
- **某個索引集的日誌，因為「另一個」索引集在同一小時有歸檔，就被當成已歸檔——實際上從未被歸檔。** 逐區塊的重複判斷使用 `is_time_range_covered(..., exclude_stream_id_prefix=<前置碼>)`，語意是「**任何** stream_id 不以此前置碼開頭的歸檔，是否涵蓋此區塊？」。在擁有 27 個索引集的站台上，`graylog_147` 的歸檔因此被當成 `filesrv_33` 的涵蓋——但兩者是完全不同的日誌來源。造成兩個都是無聲的後果：
...
- **對新名稱首次 Bulk 匯入時，間歇性的無聲資料遺失。** 即使 1.13.57 已等待索引集回報就緒，寫入仍可能在 Graylog 正在重新指向（或重建）寫入索引時開始——OpenSearch 接受了這些文件、`_bulk` 回應顯示「已寫入」，但它們隨著被取代的索引一起消失。在 e2e 中大約每三次會重現一次。等待方式現在是決定性的：`_wait_for_stable_alias()` 會持續輪詢，直到 deflector 別名**連續三次**解析到**同一個實體索引**為止，而不是睡固定秒數然後祈禱。已以五次完全乾淨環境的首次匯入驗證——每次都確實寫入 36,300 筆文件。
...
- **對新名稱的首次 Bulk 匯入仍可能因競爭條件遺失文件。** 1.13.53 已會在建立索引集後主動循環（cycle）讓 Graylog 建置，但寫入可能在不到一秒後就開始——此時 Graylog 仍在建置索引集，於是回報為「已寫入」的文件其實落在隨後被 Graylog 取代的索引中。現在 preflight 會等待索引集回報就緒（並額外緩衝數秒）後才開始寫入。已於乾淨叢集驗證：13,500 筆文件確實寫入並保留。
...
- **對新的目標索引名稱進行的「第一次」OpenSearch Bulk 匯入必定失敗。** 會中止並顯示 `Graylog deflector alias '<prefix>_deflector' does not exist on OpenSearch`。原因是建立 Graylog 索引集只會寫入 MongoDB 中繼資料，真正的第一個 OpenSearch 索引與其 deflector 別名要等到 deflector 被**循環（cycle）**才會建立，而先前沒有任何步驟會觸發它，因此單純等待永遠不會成功。現在 preflight 會在建立新索引集後主動循環它，Bulk 端也會等待別名出現（最多 60 秒）而非第一次探測就失敗。已於乾淨叢集驗證：對全新名稱的首次匯入會正確建立索引並寫入全部 6,400 筆文件。
...
- **`scripts/e2e-archive-test.sh` 會自行清理測試產物**，以產品本身的 `streams-cleanup` 移除 Bulk 步驟建立的串流與索引集。先前每執行一次就會永久遺留一組——而且在 1.13.51 之前，該指令根本刪不掉它們。
